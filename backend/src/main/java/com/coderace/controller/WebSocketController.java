...
public class WebSocketController {
        ...
        public void startGame(@DestinationVariable String roomId,
                ...
                String sessionId = headerAccessor.getSessionId();
                log.info("Start game request for room {} with request: {} by session {}",
                                roomId, request, sessionId);

                boolean started;

                // Check if using two-step generation with pre-generated problem
                if (request.getProblemId() != null && !request.getProblemId().isBlank()) {
                        log.info("Using pre-generated problem {} for room {}", request.getProblemId(), roomId);

                        // Retrieve the cached problem
                        com.coderace.model.Problem problem = gameService.getCachedProblem(request.getProblemId());

                        if (problem == null) {
                                log.error("Pre-generated problem {} not found in cache", request.getProblemId());
                                ...
                                if (room != null) {
                                        GameStateUpdate errorUpdate = new GameStateUpdate(
                                                        room,
                                                        GameConstants.MSG_ERROR_PROBLEM_NOT_FOUND);
                                        messagingTemplate.convertAndSend("/topic/room/" + roomId, errorUpdate);
                                }
                                return;
                        }

                        // Start game with pre-generated problem
                        started = gameService.startGame(roomId, sessionId, problem);

                        // Clean up: Remove from cache after successful use to prevent memory leak
                        if (started) {
                                gameService.removeCachedProblem(request.getProblemId());
                        }
                } else {
                        log.info("No problemId provided, using legacy generation with default filter");

                        // Legacy: Generate problem on-the-fly with default filter
                        com.coderace.dto.ProblemFilter filter = com.coderace.dto.ProblemFilter.defaultFilter();
                        started = gameService.startGame(roomId, sessionId, filter);
                }
                ...
                GameStateUpdate update = new GameStateUpdate(
                                room,
                                GameConstants.MSG_GAME_STARTED);
                ...
                try {
                        ...
                        GameStateUpdate update = new GameStateUpdate(
                                        ...
                                        verdict.isAccepted() ? "Code accepted! You won!"
                                                        : String.format("Tests passed: %d/%d", verdict.getTestsPassed(),
                                                                        verdict.getTotalTests()));

                        messagingTemplate.convertAndSend("/topic/room/" + roomId, update);

                        log.info("Submission processed for room {}: verdict={}, {}/{} tests passed",
                                        roomId, verdict.getVerdict(), verdict.getTestsPassed(),
                                        verdict.getTotalTests());
